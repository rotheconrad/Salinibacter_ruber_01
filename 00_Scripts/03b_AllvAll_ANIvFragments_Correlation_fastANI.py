...
'''fastANI All vs All ANI vs Shared Fragment Correlation.

This script reads an all vs all file of fastANI outputs, computes the
...
def ANI_vs_shared_plot(df, org, op):
    ...
    # Grab primary population
    df_primary = df[(df['Comp'] == 'Draft-Draft') & (df['xs'] > 97.0)]

    # Gather Stats
    df_stats_primary = gather_stats(df_primary, org, op)
    df_stats_total = gather_stats(df, org, op)
    ...
    stats_line2 = (
        f"{df_stats_primary['pcorr']}\n"
        f"{df_stats_primary['scorr']}\n"
        f"{df_stats_primary['kcorr']}\n\n"
        f"{df_stats_total['pcorr']}\n"
        f"{df_stats_total['scorr']}\n"
        f"{df_stats_total['kcorr']}"
        )
    ...
    colors = ['#252525', '#0571b0', '#4dac26', '#d01c8b']
    ...
    g.ax_joint.yaxis.grid(
        which="minor", color='#d9d9d9', linestyle='--', linewidth=1
        )
    ...
    g.ax_joint.xaxis.grid(
        which="major", color='#f0f0f0', linestyle='-', linewidth=2
        )
    ...
    mn = g.ax_joint.axvline(
        x=df_stats_primary['ani_mean'], ymin=0, ymax=1,
        color=vline_color, linewidth=2, linestyle='--',
        label='Mean'
        )
    md = g.ax_joint.axvline(
        x=df_stats_primary['ani_median'], ymin=0, ymax=1,
        color=vline_color, linewidth=2, linestyle=':',
        label='Median'
        )
    _ = g.ax_joint.axhline(
        y=df_stats_primary['frag_mean'], xmin=0, xmax=1,
        color=vline_color, linewidth=3, linestyle='--',
        )
    _ = g.ax_joint.axhline(
        y=df_stats_primary['frag_median'], xmin=0, xmax=1,
        color=vline_color, linewidth=3, linestyle=':',
        )
...
def gather_data(ani):
    ...
    ----------
    ani : str
        This is the all vs all fast ANI output file
    ...
    with open(ani, 'r') as f:
        ...
        for l in f:
            X = l.rstrip().split('\t')
            n1 = X[0].split('_')[2].split('.')[0]
            n2 = X[1].split('_')[2].split('.')[0]
