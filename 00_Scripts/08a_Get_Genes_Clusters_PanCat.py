...
'''Generate Genes Clusters PanCats List from PGE

Takes the *_binary.tsv file and the *.fnn.clstr file from the 05 PGE
Analysis program as input, and outputs a list of
Gene_Name Cluster_Name Pangenome_Category (Core, Common, Rare, Specific)
...
def get_category(bnry):
    ...
    with open(bnry, 'r') as f:
        ...
        for l in f:
            X = l.rstrip().split('\t')
            cluster = X[-1] # Cluster # is the last column of bnry
            gs = [int(i) for i in X[1:-1]]
